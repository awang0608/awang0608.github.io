...
参考地址：[最详尽的 JS 原型与原型链终极详解，没有「可能是」](https://www.jianshu.com/p/dee9f8b14771)
    ...
    __proto__的内置属性，用于指向创建它的构造函数的原型对象，而原型对象又有自己的原型对象，这样一连串的
    __proto__指向就构成了原型链，直到值为null。通过原型链我们可以进行两个类型之间的继承
...
> IE和火狐的事件机制有什么区别
    ...
    获，若为false，则采用事件冒泡，IE浏览器只支持事件冒泡，不支持事件捕获，所以它不支持该方法，所以
...
> 任务的执行模式
    ...
    处理异步的几种方法：
        ...
        维护，各个部分之间高度耦合（Coupling），流程会很混乱回调并不一定就是异步。他们自己并没有直接关
        系
