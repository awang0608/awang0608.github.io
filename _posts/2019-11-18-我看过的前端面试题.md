...
> 跨域的几种方式
    ...
    3. Cross-Origin Resource Sharing（CORS）跨域资源共享（服务器一般需要增加响应头），浏览器向跨源服务
    器发出XMLHttpRequest请求，从而克服了AJAX只能同源使用的限制，所有浏览器都支持该功能，IE浏览器不能低
    于IE10，CORS支持所有类型的HTTP请求（后台中加上响应头来允许域请求Access-Control-Allow-Origin）
    ...
    无法利用document.domain进行跨域，所以只能主域相同而子域不同的情况下使用，两个页面通过js强制设置document.domain
    为基础主域，就实现了同域
